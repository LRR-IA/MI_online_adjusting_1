...
from helpers.brain_data import Offline_read_csv, brain_dataset, brain_dataset_weight, Online_read_csv, Online_simulation_read_csv, Online_simulation_read_csv_windows_preprocess_normalization
    ...
    accuracy_perclass_save2csv, accuracy_perclass_iteration_plot, eval_model_fea_exemplars, eval_model_fea_exemplars_distillation, train_one_epoch_fea_distillation,\
        eval_model_fea_exemplars_distillation_label, eval_model_fea_exemplars_distillation_datafea_logitlabel, train_one_epoch_logit_distillation, train_one_epoch_label_distillation, \
            train_one_epoch_logitlabel_distillation, eval_model_fea_loss, train_one_epoch_fea_selfpace, train_one_epoch_fea_selfpace_quantiles, train_one_epoch_fea_selfpace_weights, train_one_epoch_fea_selfpace_quantiles_rank, \
                eval_model_fea_lossWeigt_selfpace, train_one_epoch_fea_weighted
    ...
    for trial_idx in range(trial_nums):
        ...
        if (trial_idx + 1) % update_wholeModel == 0:           
            ...
            for epoch in trange(_n_epoch_online, desc='online classification update'):
                ...
                # update lambda_pace for this epoch (classic SPL: λ increases across epochs)
                lambda_pace = min(initial_A + delta_A * (epoch + 1), 1.0)
                lambda_pace_new = 1.0 * lambda_pace

                # first, calculate the loss of each sample for selection via quantile lambda of each class
                criterion = nn.CrossEntropyLoss(reduction='none')  # set the CE loss mode
                selected_data_batches, selected_labels, weights = eval_model_fea_lossWeigt_selfpace(model, sub_online_train_loader, criterion, device, lambda_pace_new)
                
                # second, calculate the weight for the selected samples
                ...
                # finally, tune the model with the weights and samples
                average_loss_this_epoch = train_one_epoch_fea_weighted(model, optimizer, criterion, selected_data_weights_loader, device)
                criterion = nn.CrossEntropyLoss()
                val_accuracy, _, _ , _, loss_avg_val = eval_model_fea_loss(model, sub_online_val_loader, criterion, device)
