...
def SeverDataProcessingOnline(ip='172.18.22.21', port=8888):
...
def SeverDataReceive(ip='172.18.22.21', port=8888):
    ...
    while True:
        ...
        while True:
            ...
            width = int(decode[2])
            echo = decode[3:int(height*width+3)]
            mat = echo.reshape((height,width),order='F')  # 接收到的数据，矩阵形式，大小(height * width)
            break
        break
    return tcpCliSock, addr, mat

def ServerDataSend(tcpCliSock, addr, data, ip='172.18.22.21', port=8888):
