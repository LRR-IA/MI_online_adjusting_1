...
# transform the data into the form of mne type class and adopt the channel selection and normalization
# it is the demo of creating a new dataset, it will be modified later
...
TMIN = 0
TLEN = 1.0  # the start and end of the event
...
def create_dummy_raw():
    ...
    raw = mne.io.RawArray(data, info)  # create the information and data for mne
...
def create_dummy_dataset(epoched=True, sessions_per_thinker=2, num_thinkers=THINKERS_IN_DATASETS,
    ...
    return Dataset({"p{}".format(i): thinker.clone() for i in range(num_thinkers)}, **dataset_args)  # Dataset: thinkers: sessions
