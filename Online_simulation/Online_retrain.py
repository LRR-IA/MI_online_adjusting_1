            ...
            if (trial_idx+1) % update_wholeModel == 0:
                ...
                for lr in lrs:
                    for dropout in dropouts:    
                        ...
                        whole_model_best_val_accuracy = 0.0
                        epoch_train_loss = []
                        epoch_train_accuracy = []
                        epoch_validation_accuracy = []
                        ...
                        for epoch in trange(_n_epoch_online, desc='online classification update whole model'):
                            ...
                            if whole_model_is_best:
                                ...
                                print("New best model found at epoch {}, val accuracy: {}".format(epoch, whole_model_val_accuracy))
                                torch.save(model.state_dict(), os.path.join(result_save_subject_checkpointdir, 'best_model.pt'))
                            ...
                            if epochs_no_improve >= args_dict.patience:  # early stopping after several epochs with no improvement
                                print("Early stopping at epoch {} due to no improvement in validation accuracy.".format(epoch))
                                break
