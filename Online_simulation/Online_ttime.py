...
def Online_updating_EEGNet_simulation(args_dict):
    ...
    # paper: Li S, Wang Z, Luo H, et al. T-TIME: Test-time information maximization ensemble for plug-and-play BCIs[J]. IEEE Transactions on Biomedical Engineering, 2023.
    for trial_idx in range(trial_nums):
        ...
        if (trial_idx + 1) % update_trial == 0 and len(feature_queue) == queue_size:           
            ...
            # =========================================================================================================
            # T-TIME method parameter update (Phase 2), without IEA
            # Construct batch_test directly from the FIFO queue
            batch_test = np.concatenate(feature_queue, axis=0)
            batch_test = torch.from_numpy(batch_test).to(torch.float32).to(device)

            model.train()
            optimizer = torch.optim.Adam(model.parameters(), lr=args_dict.lr_online if hasattr(args_dict, 'lr_online') else lr)

            model_k = copy.deepcopy(model)

            # update target model
            # T-TIME original uses steps=1 (single gradient step per update, lightweight TTA)
            steps = 1
            epsilon = 1e-5
            t = 2.0  # temperature rescaling parameter

            for step in range(steps):
                outputs, feature = model(batch_test)
                
                # Softmax with temperature rescaling
                softmax_out = torch.nn.Softmax(dim=1)(outputs / t)
                
                # Conditional Entropy Minimization (CEM)
                CEM_loss = torch.mean(torch.sum(-softmax_out * torch.log(softmax_out + epsilon), dim=1))
                
                # Marginal Distribution Regularization (MDR)
