...
import torch.nn as nn
from scipy.linalg import sqrtm, inv as sp_inv
...
def online_euclidean_alignment(x, running_mean, n_samples):
    """
    在线欧氏对齐（Online EA）
    x: np.ndarray, shape (batch, channels, time)
    running_mean: 运行均值协方差矩阵 shape (channels, channels)，初始传 None
    n_samples: 已累积样本数，初始传 0
    返回: 对齐后的 x (同 shape)，更新后的 running_mean，更新后的 n_samples
    """
    n, c, t = x.shape

    # 计算当前 batch 每个样本的协方差，取均值
    batch_covs = [x[i] @ x[i].T / t for i in range(n)]
    batch_mean = np.mean(batch_covs, axis=0)  # (channels, channels)

    # 在线递增更新运行均值
    if running_mean is None:
        running_mean = batch_mean
    else:
        running_mean = (n_samples * running_mean + n * batch_mean) / (n_samples + n)
    n_samples += n

    # 计算对齐矩阵 R^{-1/2}
    try:
        R_inv_sqrt = np.real(sp_inv(sqrtm(running_mean + 1e-8 * np.eye(c))))
    except Exception:
        R_inv_sqrt = np.eye(c)

    # 对每个样本做对齐：x_aligned = R^{-1/2} @ x
    x_aligned = np.stack([R_inv_sqrt @ x[i] for i in range(n)], axis=0)

    return x_aligned, running_mean, n_samples


def online_adabn(model, x_np, device):
    """
    在线 AdaBN：用当前数据更新模型所有 BatchNorm2d 层的统计量
    x_np: np.ndarray, shape (batch, channels, time)
    模型 forward 内部会 unsqueeze(1)，所以直接传 3D 即可
    """
    # 将所有 BN 层切换为 train 模式，其余保持 eval
    for module in model.modules():
        if isinstance(module, nn.BatchNorm2d):
            module.train()
            module.momentum = None  # 使用累积移动平均而非固定 momentum

    x_tensor = torch.tensor(x_np.astype(np.float32)).to(device)  # (batch, channels, time)

    with torch.no_grad():
        _ = model(x_tensor)  # 触发 BN 统计量更新，返回值不用
...
def joint_sequential_finetune(model, feature_buffer, label_buffer, device, lr=0.001, n_epochs=10, batch_size=16):
    """
    Joint Sequential Fine-tuning
    - joint：使用 feature_buffer 中所有历史数据拼接训练
    - sequential：直接在当前 model 权重基础上继续训练，不重置
    feature_buffer: list of np.ndarray, 每个元素 shape (n, channels, time)
    label_buffer:   list of np.ndarray, 每个元素 shape (n,)
    """
    X = np.concatenate(feature_buffer, axis=0).astype(np.float32)  # (N, channels, time)
    y = np.concatenate(label_buffer, axis=0).astype(np.int64)       # (N,)

    dataset = brain_dataset(X, y)
    loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True)

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=lr)

    print(f"--- Starting Joint Sequential Fine-tuning ({X.shape[0]} samples) ---")
    for epoch in range(n_epochs):
        average_loss_this_epoch = train_one_epoch_fea(model, optimizer, criterion, loader, device)
        train_accuracy, _, _, _, _, accuracy_per_class = eval_model_confusion_matrix_fea(model, loader, device)
        
        print(f"Epoch [{epoch + 1}/{n_epochs}] - Train Loss: {average_loss_this_epoch:.4f} | "
              f"Train Acc: {train_accuracy:.2f}% | Class Accs: {np.round(accuracy_per_class, 4)}")
...
def Online_updating_EEGNet_simulation(args_dict):
    ...
    use_ea = getattr(args_dict, 'use_ea', False)
    ...
    # ---- Joint Sequential Fine-tuning + OTTA 初始化 ----
    # EA 运行状态（跨整个 session 持续累积，session 结束后随模型一起重置）
    ea_running_mean = None
    ea_n_samples = 0

    # joint buffer：累积所有历史 trial 数据
    joint_feature_buffer = []
    joint_label_buffer = []

    # finetuned_state_dict：保存最新 finetune 后的干净权重
    # 对应论文逻辑：
    #   - session 开始时加载上一个 session finetune 后的权重，整个 session 内 AdaBN 持续累积
    #   - session 结束时，重新从 finetuned_state_dict 加载干净权重，做 finetune，更新 finetuned_state_dict
    finetuned_state_dict = copy.deepcopy(model.state_dict())

    # session 开始时加载 finetune 权重（第一个 session 直接用离线模型）
    model.load_state_dict(copy.deepcopy(finetuned_state_dict))
    model = model.to(device)
    print("--- Session 1 started, loaded initial offline model weights ---")
    ...
    # this is the implementation of the TTIME
    # Wimpff M, Aristimunha B, Chevallier S, et al. Fine-tuning strategies for continual online EEG motor imagery decoding: insights from a large-scale longitudinal study[C]//2025 47th Annual International Conference of the IEEE Engineering in Medicine and Biology Society (EMBC). IEEE, 2025: 1-7.
    for trial_idx in range(trial_nums):
        ...
        # ---- 不在每个 trial 重置模型权重 ----
        # AdaBN 跨 trial 持续累积，对应论文中 session 内连续适应的设计
        # 权重重置只发生在：(1) session 开始时加载 finetune 权重；(2) finetune 前恢复干净权重
        model = model.to(device)

        ground_truth_label = np.unique(sub_train_label_batches)
        print("ground truth label:{}".format(ground_truth_label))

        # ---- 逐样本 OTTA（单样本级别，跨 trial 持续累积）----
        # 每个样本依次经过：EA对齐（可选）→ AdaBN → 推理
        # AdaBN 状态在整个 session 内连续累积，不在 trial 边界重置
        for sample_idx in range(sub_train_feature_batches.shape[0]):
            # 取单个样本，shape: (1, channels, time)
            single_sample = sub_train_feature_batches[sample_idx:sample_idx+1]

            # OTTA Step 1: 在线欧氏对齐（EA，可选）
            if use_ea:
                single_sample, ea_running_mean, ea_n_samples = online_euclidean_alignment(
                    single_sample, ea_running_mean, ea_n_samples
                )

            # OTTA Step 2: 逐样本 AdaBN，在当前模型基础上更新 BN 统计量
            model = online_adabn(model, single_sample, device)

        # ---- 用本 trial 最后一次 AdaBN 适应后的模型对整个 trial 做推理 ----
        sub_train_feature_batches_aligned = sub_train_feature_batches
        _sub_updating_predict = brain_dataset(sub_train_feature_batches_aligned, sub_train_label_batches)
        sub_updating_predict = torch.utils.data.DataLoader(
            _sub_updating_predict,
            batch_size=sub_train_feature_batches_aligned.shape[0],
            shuffle=False
        )
        predict_accu, class_predictions_array, labels_array, probabilities_array, _, accuracy_per_class = eval_model_confusion_matrix_fea(
            model, sub_updating_predict, device
        )
        ...
        if (trial_idx + 1) % update_trial == 0:           
            ...
            # 1. 将本轮 update_trial 个 trial 的新数据存入 joint buffer
            start_idx = (trial_idx + 1 - update_trial) * batch_size_online
            end_idx = (trial_idx + 1) * batch_size_online
            joint_feature_buffer.append(sub_train_feature_array_1[start_idx:end_idx].astype(np.float32))
            joint_label_buffer.append(sub_train_label_array_1[start_idx:end_idx])

            # 2. session 结束：重新加载上一个 session 的干净 finetune 权重
            # 丢弃本 session 内 AdaBN 累积的状态，与 OTTA 完全解耦
            model.load_state_dict(copy.deepcopy(finetuned_state_dict))
            model = model.to(device)
            print(f"--- Session ended at trial {trial_idx + 1}, reloaded clean finetune weights for finetune ---")

            # 3. Joint Sequential Fine-tuning（sequential：从上一次 finetune 权重出发，不重置到离线模型）
            model = joint_sequential_finetune(
                model=model,
                feature_buffer=joint_feature_buffer,
                label_buffer=joint_label_buffer,
                device=device,
                lr=lr,
                n_epochs=n_epoch_online,
                batch_size=batch_size_online
            )

            # 4. 更新 finetuned_state_dict，作为下一个 session OTTA 和 finetune 的新起点
            finetuned_state_dict = copy.deepcopy(model.state_dict())
            torch.save(
            ...
            # 5. 下一个 session 开始：加载新的 finetune 权重，AdaBN 从干净状态重新累积
            model.load_state_dict(copy.deepcopy(finetuned_state_dict))
            model = model.to(device)
            ea_running_mean = None   # EA 统计量也随 session 重置
            ea_n_samples = 0
            print(f"--- Session {trial_idx // update_trial + 2} started, loaded new finetuned weights ---")
            print(f"Model updated at trial {trial_idx + 1}, online buffer size: {sum(len(f) for f in joint_feature_buffer)} samples")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_ea', default=False, type=str2bool, help="whether to use Euclidean Alignment")
