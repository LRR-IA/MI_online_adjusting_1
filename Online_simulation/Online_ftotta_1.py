...
from helpers.tta_adapters import Alignment, Norm, RobustBN
...
def Online_updating_EEGNet_simulation(args_dict):
    ...
    # ─── OTTA + Joint Sequential Fine-tuning 初始化 ───
    otta_config = {
        "alignment": "euclidean" if use_ea else False,
        "alpha": 0.001,
    }
    ...
    def make_otta_model(base_model, config):
        return Norm(copy.deepcopy(base_model), config=config)

    # model 已经加载了离线权重，直接构建第一个 session 的 OTTA 模型
    model = model.to(device)
    otta_model = make_otta_model(model, otta_config)
    ...
    # this is the implementation of the finetune+otta
    ...
    for trial_idx in range(trial_nums):
        ...
        # ─── 逐样本 OTTA：自适应与推理同步进行 ───
        # 每个样本一次 forward = EA更新+白化(可选) + RobustBN统计量更新 + 推理
        # 与论文 causal_evaluation.py 的 predict_dataloader(batch_size=1) 完全等价
        all_logits = []
        all_labels = []
        with torch.no_grad():
            for sample_idx in range(sub_train_feature_batches.shape[0]):
                single_np = sub_train_feature_batches[sample_idx:sample_idx + 1]
                x_tensor = torch.tensor(single_np.astype(np.float32)).to(device)
                # 单次 forward：自适应状态更新 + 推理同时完成，返回 logits (1, num_classes)
                logits = otta_model(x_tensor)
                all_logits.append(logits.cpu())
                all_labels.append(int(sub_train_label_batches[sample_idx]))

        # ─── 用本 trial 各样本的 OTTA 输出统一计算精度 ───
        all_logits_np = torch.cat(all_logits, dim=0).numpy()
        all_labels_np = np.array(all_labels)
        class_predictions_array = all_logits_np.argmax(axis=1)
        predict_accu = (class_predictions_array == all_labels_np).mean() * 100

        probabilities_array = all_logits_np
        labels_array        = all_labels_np

        conf_matrix      = sklearn_cm(all_labels_np, class_predictions_array)
        class_accuracies = conf_matrix.diagonal() / conf_matrix.sum(axis=1).clip(min=1)
        ...
        if (trial_idx + 1) % update_wholeModel == 0:           
            ...
            # 1. 将本轮新数据存入 joint buffer
            start_idx = (trial_idx + 1 - update_wholeModel) * batch_size_online
            end_idx   = (trial_idx + 1) * batch_size_online
            joint_feature_buffer.append(
                sub_train_feature_array_1[start_idx:end_idx].astype(np.float32))
            joint_label_buffer.append(
                sub_train_label_array_1[start_idx:end_idx])

            # 2. session 结束：重新加载干净的 finetune 权重，与 OTTA 累积状态解耦
            model.load_state_dict(copy.deepcopy(finetuned_state_dict))
            model = model.to(device)
            print(f"--- Session ended at trial {trial_idx + 1}: reloaded clean weights for finetune ---")

            # 3. Joint Sequential Fine-tuning（sequential：从上一次 finetune 权重出发）
            ...
            # 4. 更新 finetuned_state_dict，作为下一个 session 的新起点
            finetuned_state_dict = copy.deepcopy(model.state_dict())
            torch.save(
            ...
            # 5. 下一个 session：直接用 FT 后的 model 构建新 OTTA 模型
            # model 此时就是 joint_sequential_finetune 返回的最新状态，无需重复 load_state_dict
            # 新建 Norm 对象 = EA reference/counter 清零
            #               + RobustBN source 统计量从新 finetune 权重的 BN running stats 重置
            # 对应论文 causal_evaluation.py：每个 fold load 最新 finetune ckpt 后 deepcopy 新建 tta_model
            otta_model = make_otta_model(model, otta_config)
            print(f"--- Session {trial_idx // update_wholeModel + 2} started: new OTTA model created ---")
            print(f"Online buffer: {sum(len(f) for f in joint_feature_buffer)} samples total")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size_online', default=9, type=int, help="number of batch size for online updating")
    ...
    parser.add_argument('--update_trial', default=12, type=int, help="number of trails for instant updating")
